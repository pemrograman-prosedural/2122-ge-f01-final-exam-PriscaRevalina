...
void assign_student(struct dorm_t *drm, struct student_t *mhs, char *nim, char *asrama, int zstd, int zdrm, int find_id(char *nim, int zstd, struct student_t *mhs), int find_dorm(char *asra, int zdrm, struct dorm_t *drm))
{
    int maha = find_id(nim, zstd, mhs);
    int asra = find_dorm(asrama, zdrm, drm);
